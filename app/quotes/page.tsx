...
  { id: 4, text: "It is during our darkest moments that we must focus to see the light.", author: "Aristotle", category: "Hope" },
...
  { id: 6, text: "The only impossible journey is the one you never begin.", author: "Tony Robbins", category: "Journey" }
];
...
          </Button>
          <Button onClick={getRandomQuote} disabled={isLoading} variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-50 px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2">
            <RefreshCw className={`h-5 w-5 ${isLoading ? 'animate-spin' : ''}`} />
            <span>Random Quote</span>
          </Button>
          <Button onClick={shareQuote} disabled={!currentQuote || (showGeneratedQuotes && !hasSelectedQuote)} variant="outline" className={`border-slate-300 px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 ${(!currentQuote || (showGeneratedQuotes && !hasSelectedQuote)) ? 'opacity-50 cursor-not-allowed text-slate-400' : 'text-slate-700 hover:bg-slate-50'}`}>
            <Share2 className="h-5 w-5" />
            <span>Share</span>
          </Button>
          <Button onClick={() => setIsLiked(!isLiked)} disabled={!currentQuote || (showGeneratedQuotes && !hasSelectedQuote)} variant="outline" className={`border-slate-300 px-6 py-3 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 ${(!currentQuote || (showGeneratedQuotes && !hasSelectedQuote)) ? 'opacity-50 cursor-not-allowed text-slate-400' : isLiked ? 'text-red-600 border-red-300 bg-red-50 hover:bg-red-100' : 'text-slate-700 hover:bg-slate-50'}`}>
